...
- Effect timing is prepared conceptually; v0.3.8 renders global effect strength. Per-effect start/end keyframes are planned for Phase 2.
- Batch rendering is stable in v0.3.8 with queue details, job reorder, per-job render mode, `MAX for all`, stop support, and optional two-pipeline rendering on stronger Macs.
